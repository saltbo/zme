...
function SheetOverlay({ className, ...props }: SheetPrimitive.Backdrop.Props) {
  return (
    <SheetPrimitive.Backdrop
      ...
      className={cn(
        'fixed inset-0 z-50 bg-black/10 transition-opacity duration-150 data-ending-style:opacity-0 data-starting-style:opacity-0 motion-reduce:transition-none supports-backdrop-filter:backdrop-blur-xs',
        className,
      )}
      {...props}
    />
      ...
      >
        ...
        {showCloseButton && (
          <SheetPrimitive.Close
            ...
            render={<Button variant="ghost" className="absolute top-2 right-2 size-11" size="icon-lg" />}
          >
            <XIcon />
